...
1. カード抽選、正逆位置の決定、スプレッド、配置位置はアプリ側の決定論的なロジックとする。初期ワンオラクルは現行どおり正位置のみとする。
...
`OracleCore` がカード順とカードの向きを決定する。AI呼び出しや再試行で向きを再抽選してはならない。AI基盤導入と逆位置導入は別工程とし、初期実装は `orientationMode: "upright_only"` を使用する。

抽選設定は次の形を取る。

```json
{
  "orientationMode": "upright_only",
  "reversedProbability": 0
}
```

`orientationMode` は `upright_only` または `mixed` とする。`upright_only` では全カードを正位置とし、`reversedProbability` を参照しない。将来 `mixed` を有効にした場合だけ、0以上1以下の `reversedProbability` を使用する。初期仕様として50%に固定しない。
...
フェーズ1では逆位置の抽選動作を実装対象に含めず、設定値とデータ契約だけを将来拡張可能にする。`mixed` の実装時は別の設計・テスト工程とし、乱数注入により再現可能にする。
...
サーバーは要求を検証後、`general` と `action` を常に辞書から選ぶ。`love`、`work`、`finance`、`relationships` は質問との関連に応じて0個以上を補助情報として追加できる。質問を必ず一つのカテゴリへ分類せず、恋愛と仕事が重なる質問などでは複数カテゴリを参照できる。カテゴリ推定に確信がない場合は `general` と `action` だけで構成し、入力トークンを抑える。
...
`mode` は `ai` または `fallback` とする。通常のリーディングでは `cautions` は空配列を許可する。医療、法律、投資、人命など、追加の注意が必要な質問だけに短い注意文を設定する。通常時の非断定性は定型の注意文ではなく、`summary`、`interpretation`、`advice` の文章ルールとして実現する。

UIはプロバイダー名やフォールバックの詳細理由を必要としないため、公開結果には含めない。運用ログではサーバー側だけにプロバイダー、モデル、処理時間、失敗種別、概算トークン数を記録できる設計とする。内部フォールバック理由は `disabled`、`quota`、`timeout`、`rate_limit`、`provider_error`、`invalid_output`、`offline` の列挙値で保持する。質問全文は既定で運用ログへ保存しない。
...
- 質問の具体的な状況と、カードの基本意味・行動助言がどこで接続するかを説明する。辞書文の単純な言い換えだけで終えない。
- 複数カードでは、カード同士の補強、矛盾、因果、時間的な流れ、結果が成立する条件を整理する。ワンオラクルでも将来この入力構造へ拡張できる形を維持する。
...
- AIプロバイダーのタイムアウト
- レート制限または無料枠超過
- 1日のAI総利用上限または予算クォータのサーキットブレーカー作動
- ネットワーク障害
- プロバイダーの5xx応答
- JSON解析または出力検証の失敗
- サーバー設定でAIを無効にしている場合

フォールバックは辞書とスプレッド定義から、カード名、正逆位置、基本意味、配置位置、行動アドバイスを組み立てる。乱数を使わず、同じ入力には同じ基本結果を返す。HTTP応答自体は可能な限り成功扱いとし、`mode: "fallback"` でUIへ状態を伝える。内部では理由コードを運用ログへ記録するが、通常UIには「今回は簡易リーディングを表示しています」とだけ示す。
...
- IPまたは匿名の利用単位ごとに短時間・1日単位のレート制限を設定する。
- 1日のAI総呼び出し回数またはトークン上限を設定し、予算・クォータ到達時はサーキットブレーカーを開いてAIを呼ばずフォールバックする。
- 通常と異なる大量アクセスを検出した場合はAIを呼ばずフォールバックする。
- レート制限だけで悪用を抑えられない場合はCloudflare Turnstileを追加できる入口を用意する。初期のローカル基盤にはTurnstileを必須としない。
...
- `one-oracle-flow-core.js`: 現行の正位置のみを維持し、確定結果に `orientation: "upright"` を追加できる契約へ整える。
- `one-oracle-art-ui.js`: `oracle:result` にスプレッドID、配置ID、現在の正位置を追加する。
...
- Fisher–Yates、カット、選択が維持され、フェーズ1では常に正位置になること
- AI要求が確定カードを変更しないこと
...
2. CORS、要求サイズ制限、IPまたは利用単位のレート制限、1日のAI総利用上限、予算・クォータのサーキットブレーカー、異常アクセス時フォールバック、運用ログ
3. GeminiProvider
4. 質問送信に関する案内
5. 実APIの品質・料金評価
6. 必要に応じてOpenAIProviderまたはCloudflareProvider
7. 必要に応じてCloudflare Turnstile
...
- カード抽選とAI解釈が独立している。
- 大アルカナ22枚のデータに正逆位置と指定された分野の意味があり、現行ワンオラクルの抽選は正位置のみを維持する。
