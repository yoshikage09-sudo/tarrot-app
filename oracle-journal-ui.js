...
sources.reduce((ready,src)=>ready.then(()=>new Promise((resolve,reject)=>{const script=document.createElement('script');script.src=`${src}?v=2`;script.onload=resolve;script.onerror=reject;document.body.appendChild(script)})),Promise.resolve()).catch(()=>{result.hidden=false;result.textContent='リーディング機能を準備できませんでした。ページを再読み込みしてください。'});
})();
